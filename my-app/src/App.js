...
const SignaturePadComponent = ({ onEnd, clear }) => {
  ...
  useEffect(() => {
    if (canvasRef.current) {
      ...
      signaturePadRef.current = new SignaturePad(canvas, {
        ...
        penColor: 'black',
        onEnd: () => {
          onEnd(signaturePadRef.current.toDataURL());
        }
      });
    ...
    <div className="max-w-md mx-auto bg-white rounded-xl shadow-lg">
      <div className="p-6 space-y-6">
        ...
        <button 
          ...
          disabled={!firstName || !lastName || !roomNumber || !signature || (currentMeal !== 'closed' && mealCount === 0)}
        >
          {t.submitOrder}
        </button>
      </div>
